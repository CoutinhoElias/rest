...
class ProfissoesPessoa(models.Model):# MANY_TO_MANY MANUAL
    pessoa = models.ForeignKey(Pessoa, related_name='profissaoPessoa')
    profissao = models.ForeignKey(Profissao)#, related_name='profissao'
...
class QualificacaoProfissoesPessoa(models.Model):# MANY_TO_MANY MANUAL
    ...
    def __str__(self):
        return self.qualificacao.qualificacaoprofissoespessoa_set #<<=== Profissao do class profissao equivalente ao nome da profissão
                   #self.profissao. <<=== Variavel deste class, equivalente ao id da profissao nesta tabela

    def __unicode__(self):
        return "%s is in qualificacao %s (as %s)" % (self.profissaopessoa, self.qualificacao, self.type)
